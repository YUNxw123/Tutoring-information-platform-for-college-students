    ...
    <div class="content-right">
      <div class="top-select-view flex-view">
        <div class="search-entry">
          ...
          <input placeholder="输入家教姓名关键词" ref="ss" @keyup.enter="search" />
        </div>
      </div>
...
<script setup>
  ...
  watch(
    ...
    (newVal) => {
      if (newVal) {
        //这里是数据更新变化后需要执行的动作
        contentData.selectTagId.push(newVal);
      }
    },
    () => props.cc,
    (newVal) => {
      if (newVal) {
        //这里是数据更新变化后需要执行的动作
        contentData.selectedKeys.push(newVal);
      }
    },
  ...
  const search = () => {
    getThingList({ keyword: ss.value.value });
  };
  ...
  const onSelect = (selectedKeys) => {
    ...
    if (contentData.selectedKeys.length > 0) {
      ...
      getThingList({ c: getSelectedKey(), tag: tags });
    }
  };
  const clickTag = (index) => {
    if (contentData.selectTagId.includes(index)) {
      ...
      let tags = contentData.selectTagId.join(',');
      getThingList({ tag: tags, c: getSelectedKey() });
    } else {
