  ...
  <div class="detail">
    ...
    <div class="detail-content">
      <div class="detail-content-top">
        <div class="title"> 用户需求 </div>
        <div style="position: relative">
          <div class="order-infos-view">
            <div class="order-infos">
              <div class="order-img-box">
                <img :src="detailData.useravatar" v-if="detailData.suserrole == '2'" />
                <img :src="AdminAvatar" v-else />
              </div>
              <div class="order-info-box">
                <div class="order-state">
                  <span class="state hidden-sm">信息状态</span>
                  <span v-if="detailData.status == 0">有效</span>
                  <span v-else-if="detailData.status == 3">已结束</span>
                  <span v-else>无效</span>
                </div>
                <h1 class="order-name" v-if="detailData.userrole == '2'">用户：{{ detailData.username }}</h1>
                <h1 class="order-name" v-else>管理员</h1>
                <span>
                ...
                <div class="translators flex-view" style="">
                  <span>科目：</span>
                  <span class="name" v-for="item in tagData.tags" style="margin-right: 3px">{{ item.title }}</span>
                </div>
                <div class="translators flex-view" style="">
                  <span>性别：</span>
            ...
            <div class="order-counts hidden-sm">
      ...
      <div class="detail-content-bottom">
        <div class="order-content-view flex-view">
          <div class="main-content">
            ...
            <!--简介-->
            <div class="order-intro" :class="selectTabIndex <= 0 ? '' : 'hide'">
              <p class="text" style="" v-if="detailData.description">{{ detailData.description }}</p>
              <p class="text" style="" v-else>暂无数据</p>
            </div>

            <!--评论-->
            <div class="order-comment" :class="selectTabIndex > 0 ? '' : 'hide'">
          ...
          <div class="recommend" style="">
            ...
            <div class="orders">
              <div class="order-item order-item" v-for="item in recommendData" @click="handleDetail(item)">
                <div class="img-view"> <img :src="item.useravatar" /></div>
                <div class="info-view">
                  <h3 class="order-name">{{ item.username }}</h3>
                  <span>
                    <span class="a-price-symbol"></span>
...
<script setup>
  ...
  import { detailApi, listApi as listorderList } from '/@/api/index/order';
  import { listorderCommentsApi, createApi as createCommentApi, likeApi } from '/@/api/index/comment';
  import { addWishUserApi } from '/@/api/index/order';
  ...
  const getorderDetail = () => {
    detailApi({ id: orderId.value })
      .then((res) => {
        ...
        detailData.value.useravatar = BASE_URL + detailData.value.useravatar;
        if (detailData.value.sex) {
          if (detailData.value.sex == 'M') {
            detailData.value.sex = '男';
          } else if (detailData.value.sex == 'F') {
            detailData.value.sex = '女';
          } else {
            detailData.value.sex = '无要求';
          }
        }
        listTagList({}).then((res) => {
          res.data.forEach((item) => {
            if (detailData.value.tag.includes(item.id)) {
              tagData.tags.push(item);
            } else {
              return;
            }
          });
  ...
  const addToWish = () => {
    ...
    if (username) {
      addWishUserApi({ orderId: orderId.value, username: username })
        .then((res) => {
  ...
  const handleOrder = (detailData) => {
    ...
    if (userId) {
      Modal.info({
        title: '联系方式',
        content: '联系电话：' + detailData.mobile,
        onOk() {},
      });
    } else {
      message.warn('请先登录！');
  ...
  const getRecommendorder = () => {
    listorderList({ sort: 'recommend' })
      .then((res) => {
  ...
  const sendComment = () => {
    ...
    if (userId) {
      createCommentApi({ content: text, order: orderId.value, user: userId })
        .then((res) => {
          getCommentList();
        })
        .catch((err) => {
  ...
  const getCommentList = () => {
    listorderCommentsApi({ orderId: orderId.value, order: order.value })
      .then((res) => {
