<template>
  <div class="mine-infos-view">
    <div class="info-box flex-view">
      <img :src="tData.form.avatar" class="avatar-img" v-if="tData.form.avatar" />
      <img :src="AvatarImg" class="avatar-img" v-else />
      <div class="name-box">
        <h2 class="nick" v-if="tData.form.nickname">{{ tData.form.nickname }}</h2>
        <div class="name" v-if="tData.form.nickname">
          <span>用户名：{{ userStore.user_name }}</span>
          <span class="give-point"></span>
        </div>
        <h2 class="nick" v-else>{{ userStore.user_name }}</h2>
      </div>
    </div>
    <div class="counts-view">
      <div class="counts flex-view">
        ...
        <div class="follow-box flex-item" @click="clickMenu('wishorderView')">
    ...
    <div class="setting-box">
      ...
      <div class="list">
        ...
        <div class="mine-item flex-view" @click="clickMenu('jiajiaoEditView')">
          <img :src="SettingIconImage" alt="家教入驻" />
          <span>家教入驻</span>
        </div>
        <div class="mine-item flex-view" @click="clickMenu('demandView')">
          <img :src="SettingIconImage" alt="发布需求" />
          <span>发布需求</span>
        </div>
        ...
        <div class="mine-item flex-view" @click="clickMenu('commentView')">
          <img :src="PushIconImage" alt="评论管理" />
          <span>评论管理</span>
        </div>
...
<script setup lang="ts">
  ...
  import { getWishorderListApi } from '/@/api/index/order';
  ...
  const getWishorderList = () => {
    let username = userStore.user_name;
    getWishorderListApi({ username: username })
      .then((res) => {
