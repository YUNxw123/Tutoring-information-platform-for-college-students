      ...
      <div class="collect-order-view">
        <a-spin :spinning="loading" style="min-height: 200px">
          <div class="order-list flex-view">
            <div class="order-item item-column-3" v-for="(item, index) in wishData" :key="index">
              ...
              <div class="info-view">
                <h3 class="order-name">用户{{ item.username }}</h3>
                <p class="authors" v-if="item.classification_title">分类：{{ item.classification_title }}</p>
                <p class="translators" v-if="item.price">{{ item.price }}元/时</p>
              </div>
            </div>
...
<script setup>
  ...
  import { getWishorderListApi, removeWishUserApi } from '/@/api/index/order';
  ...
  const handleClickItem = (record) => {
    let text = router.resolve({ name: 'particulars', query: { id: record.id } });
    window.open(text.href, '_blank');
  };

  const handleRemove = (record) => {
    let username = userStore.user_name;
    removeWishUserApi({ username: username, orderId: record.id })
      .then((res) => {
