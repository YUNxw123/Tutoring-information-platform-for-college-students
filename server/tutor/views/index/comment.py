...
from tutor.auth.MyRateThrottle import MyRateThrottle
...
@api_view(['GET'])
def orderlist_api(request):
    if request.method == 'GET':
        demandId = request.GET.get("orderId", None)
        order = request.GET.get("order", None)
        ...
        if demandId:
            ...
            comments = Comment.objects.select_related("order").filter(order=demandId).order_by(orderBy)
            print(comments)
            serializer = CommentSerializer(comments, many=True)
            return APIResponse(code=0, msg='查询成功', data=serializer.data)
        else:
            return APIResponse(code=1, msg='orderId不能为空')
